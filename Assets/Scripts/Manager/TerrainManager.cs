...
public class TerrainManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
           audioBoostIndex += 1;
            audioManager.boostEvolutionRTPC.SetGlobalValue(audioBoostIndex);
        }
        else
        {
            audioManager.boostEvolutionRTPC.SetGlobalValue(0);
            if (audioBoostIndex < 0)
            {
                audioBoostIndex -= 1;
            }
        }
    ...
    public void Boost(float duration)
    {
        ...
        audioManager.playerBoostAudio.Post(gameObject);
        StopAllCoroutines();
    ...
    private void AddTerrain(string tag, Vector3 position)
    {
        ...
        while(newInstance == null) {
            newInstance = ObjectPooler.Instance.SpawnFromPool(tag, position, Quaternion.identity);
